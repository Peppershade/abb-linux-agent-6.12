...
#elif defined HAVE_MAKE_REQUEST_FN_IN_QUEUE && !defined USE_BDOPS_SUBMIT_BIO
int dattobd_call_mrf(make_request_fn *fn, struct request_queue *q,
                     struct bio *bio)
{
        fn(q, bio);
...
MRF_RETURN_TYPE dattobd_snap_null_mrf(struct bio *bio){
    ...
    // printk(KERN_DEBUG "dattobd_blk_mq_submit_bio function address : %p", (dattobd_blk_mq_submit_bio));
