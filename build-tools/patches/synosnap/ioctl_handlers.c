...
static int __verify_bdev_writable(const char *bdev_path, int *out)
{
        ...
        struct file *bdev_file = bdev_file_open_by_path(bdev_path, BLK_OPEN_READ, NULL, NULL);
        if (IS_ERR(bdev_file)) {
                *out = 0;
        ...
        struct bdev_handle *handle = bdev_open_by_path(bdev_path, FMODE_READ, NULL, NULL);
        if (IS_ERR(handle)) {
                *out = 0;
...
error:
        LOG_ERROR(ret, "error during info ioctl handler");
        return ret;
}


/**
 * ioctl_dattobd_lowlevel_info() - Stores relevant, current &struct snap_device lowlevel state
 *                        in @lowlevel_info.
 *
 * @info: A @struct dattobd_lowlevel_info object pointer.
 *
 * Return:
 * * 0 - successful.
 * * !0 - errno indicating the error.
 */
static int ioctl_dattobd_lowlevel_info(struct dattobd_lowlevel_info *lowlevel_info)
{
        int ret;
        struct snap_device *dev;

        LOG_DEBUG("received dattobd lowlevel info ioctl - %u", lowlevel_info->minor);


        // verify that the minor number is valid
        ret = verify_minor_in_use(lowlevel_info->minor);
        if (ret) goto error;

        dev = snap_devices[lowlevel_info->minor];
        if (tracer_read_fail_state(dev)) {
                ret = -EINVAL;
                LOG_ERROR(ret, "device specified is in the fail state");
                goto error;
        }
        if (!test_bit(SNAPSHOT, &dev->sd_state) || test_bit(UNVERIFIED, &dev->sd_state)){
                ret = -EINVAL;
                LOG_ERROR(ret, "device specified is not in active or dormant snapshot mode");
                goto error;
        }

        tracer_dattobd_lowlevel_info(dev, lowlevel_info);

        return 0;

error:
        LOG_ERROR(ret, "error during lowlevel info ioctl handler");
        return ret;
}


static int ioctl_sync_cow(unsigned int minor)
{
        int ret;
        struct snap_device *dev;

        LOG_DEBUG("received sync cow ioctl - %u", minor);
        ...
        if (!test_bit(ACTIVE, &dev->sd_state)){
                ret = -EINVAL;
                LOG_ERROR(ret, "device specified is not in active mode");
                goto error;
        }

        ret = tracer_sync_cow(dev);
        if (ret)
                goto error;

        return 0;
error:
        LOG_ERROR(ret, "error during sync cow ioctl handler");
        return ret;
...
long ctrl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
        ...
        struct dattobd_lowlevel_info *lowlevel_info = NULL;
        ...
        case IOCTL_DATTOBD_INFO:
                ...
                break;
        case IOCTL_DATTOBD_LOWLEVEL_INFO:
                // get params from user space
                lowlevel_info = kmalloc(sizeof(struct dattobd_lowlevel_info), GFP_KERNEL);
                if (!lowlevel_info) {
                        ret = -ENOMEM;
                        LOG_ERROR(ret,
                                  "error allocating memory for dattobd lowlevel info");
                        break;
                }

                ret = copy_from_user(lowlevel_info, (struct dattobd_lowlevel_info __user *)arg,
                                     sizeof(struct dattobd_lowlevel_info));
                if (ret) {
                        ret = -EFAULT;
                        LOG_ERROR(ret, "error copying dattobd lowlevel info struct from "
                                       "user space");
                        break;
                }

                ret = ioctl_dattobd_lowlevel_info(lowlevel_info);
                if (ret)
                        break;

                ret = copy_to_user((struct dattobd_lowlevel_info __user *)arg, lowlevel_info,
                                   sizeof(struct dattobd_lowlevel_info));
                if (ret) {
                        ret = -EFAULT;
                        LOG_ERROR(
                                ret,
                                "error copying dattobd lowlevel info struct to user space");
                        break;
                }
