		...
		struct ftrace_ops *ops, struct ftrace_regs *fregs)
{
	...
	struct ftrace_hook *hook = container_of(ops, struct ftrace_hook, ops);
        bool is_within_module = dattobd_within_module(parent_ip, THIS_MODULE);
        LOG_DEBUG("ftrace_callback_handler -- is_within_module : %d", is_within_module);

        LOG_DEBUG("the hook address is : %lx", hook->address);
        LOG_DEBUG("the hook function is : %p", hook->function);
#if USE_FENTRY_OFFSET
