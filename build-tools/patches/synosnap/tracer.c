...
// lowlevel io realted 
static int string_concat(const char *pathname1, const char *pathname2, char **path_out){
	int pathname1_len, pathname2_len;
	char *full_pathname;

	pathname1_len = strlen(pathname1);
	pathname2_len = strlen(pathname2);

	full_pathname = kmalloc(pathname1_len + pathname2_len + 1, GFP_KERNEL);
	if(!full_pathname){
		LOG_ERROR(-ENOMEM, "error allocating buffer for pathname concatenation");
		*path_out = NULL;
		return -ENOMEM;
	}
	full_pathname[pathname1_len + pathname2_len] = '\0';

	strncpy(full_pathname, pathname1, pathname1_len);
	strncpy(full_pathname + pathname1_len, pathname2, pathname2_len);
...
static int __clean_lowlevel_io(struct snap_device *dev)
{
	struct lowlevel_io_meta *meta_tail = dev->sd_lowlevel_io->meta;
	char *page_buf = NULL;
	int index = 0, ret = 0;

	while (meta_tail) {
		meta_tail->used.size = 0;
		memset(meta_tail->addr, 0, PAGE_SIZE);

		meta_tail = meta_tail->next;
	}
	...
	if(!page_buf){
		ret = -ENOMEM;
		LOG_ERROR(ret, "failed to allocate zeroed page");
		goto error;
	}

	for (; index < LOWLEVEL_IO_META_COUNT; index++) {
		ret = file_write(dev->sd_lowlevel_io->filp, page_buf, PAGE_SIZE * index, PAGE_SIZE);
		if(ret){
			LOG_ERROR(ret, "failed to write zeroed page to lowlevel io file");
			goto error;
		}
	}
...
static void __tracer_copy_lowlevel_io(const struct snap_device *src, struct snap_device *dest){
	dest->sd_lowlevel_io = src->sd_lowlevel_io;
}

static int __open_lowlevel_io_file(const char *cow_path, struct file **filp)
{
	int i, len, ret;
	char *file_path, *lowlevel_io_path;

	file_path = NULL;
	lowlevel_io_path = NULL;

	for(i=strlen(cow_path)-1;i >= 0; i--){
		if(cow_path[i] != '/') {
			continue;
		}

		len = i + 1;
		file_path = kzalloc(len + 1, GFP_KERNEL);

		if(!file_path){
			ret = -ENOMEM;
			LOG_ERROR(ret, "failed to allocate file path");
			goto error;
		}

		memcpy(file_path, cow_path, len);
		break;
	}

	if(!file_path){
		ret = -EINVAL;
		LOG_ERROR(ret, "invalid cow path");
		goto error;
	}

	ret = string_concat(file_path, ".synosnap.raw", &lowlevel_io_path);
	if(ret){
		LOG_ERROR(ret, "failed to concate lowlevel path");
		goto error;
	}

	ret = file_open(lowlevel_io_path, O_CREAT, filp);
	if(ret){
		LOG_ERROR(ret, "failed to open lowlevel file");
		goto error;
	}

	kfree(lowlevel_io_path);
	kfree(file_path);
	return 0;

error:
	if(lowlevel_io_path) kfree(lowlevel_io_path);
...
static void __tracer_destroy_lowlevel_io(struct snap_device *dev){
	struct lowlevel_io_meta *meta_tail = NULL;
	struct lowlevel_io_range *range_tail = NULL;

	if(!dev->sd_lowlevel_io) return;
	if(dev->sd_lowlevel_io->filp) file_unlink_and_close_force(dev->sd_lowlevel_io->filp);

	meta_tail = dev->sd_lowlevel_io->meta;
	range_tail = dev->sd_lowlevel_io->range;

	while (meta_tail) {
		if (meta_tail->page) __free_page(meta_tail->page);
		meta_tail = meta_tail->next;
		kfree(dev->sd_lowlevel_io->meta);
		dev->sd_lowlevel_io->meta = meta_tail;
	}

	while (range_tail) {
		range_tail = range_tail->next;
		kfree(dev->sd_lowlevel_io->range);
		dev->sd_lowlevel_io->range = range_tail;
	}

	kfree(dev->sd_lowlevel_io);
	dev->sd_lowlevel_io = NULL;
}

static int tracer_setup_lowlevel_io(struct snap_device *dev, const char *cow_path, int need_write_page)
{
	int ret;
	char *page_buf = NULL;
	int index = 0, meta_idx = 0, block_cnt = 0;
	// how many continueous blocks we can used to store data
	int data_cnt = 0;
	struct lowlevel_io_meta *meta_tail = NULL;
	struct lowlevel_io_range *range_tail = NULL;
	struct super_block *sb = NULL;
	sector_t pre_block = 0;

	dev->sd_lowlevel_io = kzalloc(sizeof(struct lowlevel_io), GFP_KERNEL);
	if(!dev->sd_lowlevel_io){
		ret = -ENOMEM;
		LOG_ERROR(ret, "failed to allocate lowlevel io");
		goto error;
	}
	// default value (block size = page size = 4K)
	dev->sd_lowlevel_io->max_pair_in_meta = 255; // 2040 / 8
	dev->sd_lowlevel_io->blocks_per_page = 1;
	ret = __open_lowlevel_io_file(cow_path, &dev->sd_lowlevel_io->filp);
	if(ret){
		LOG_ERROR(ret, "failed to open lowlevel io file");
		goto error;
	}

	sb = dattobd_get_super(dev->sd_base_dev);
	if(sb){
		dev->sd_lowlevel_io->block_size = sb->s_blocksize;
		LOG_DEBUG("super block size :  %lu", dev->sd_lowlevel_io->block_size);
		dev->sd_lowlevel_io->blocks_per_page = PAGE_SIZE / dev->sd_lowlevel_io->block_size;
		LOG_DEBUG("blocks_per_page :%d", dev->sd_lowlevel_io->blocks_per_page);
		dattobd_drop_super(sb);
	} else {
		LOG_WARN("cant get super block, set meta & range as null, using lowlevel io will cause error");
		dev->sd_lowlevel_io->meta = NULL;
		dev->sd_lowlevel_io->range = NULL;
		goto out;
	}
	...
	if(need_write_page){
		// allocate 2048 pages
		ret = file_allocate(dev->sd_lowlevel_io->filp, 0, PAGE_SIZE * (LOWLEVEL_IO_DATA_COUNT + LOWLEVEL_IO_META_COUNT));
		if(ret){
			LOG_ERROR(ret, "failed to allocate lowlevel io file size");
			goto error;
		}
		page_buf = (char *)get_zeroed_page(GFP_KERNEL);
		if(!page_buf){
		...
		for (; index < LOWLEVEL_IO_META_COUNT + LOWLEVEL_IO_DATA_COUNT; index++) {
			ret = file_write(dev->sd_lowlevel_io->filp, page_buf, PAGE_SIZE * index, PAGE_SIZE);
			if(ret){
				LOG_ERROR(ret, "failed to write zeroed page to lowlevel io file");
				goto error;
	...
	for (; meta_idx < LOWLEVEL_IO_META_COUNT; meta_idx++) {
		struct lowlevel_io_meta *meta = kzalloc(sizeof(struct lowlevel_io_meta), GFP_KERNEL);

		if (!dev) {
			ret = -ENOMEM;
			LOG_ERROR(ret, "failed to allocate lowlevel meta");
			goto error;
		}

		meta->page = alloc_page(GFP_NOIO);
		meta->addr = page_address(meta->page);
		meta->used.size = 0;

		if (!dev->sd_lowlevel_io->meta) {
			dev->sd_lowlevel_io->meta = meta;
			meta_tail = meta;
		} else {
			meta_tail->next = meta;
			meta_tail = meta;
	...
	for (;;index++) {
		sector_t block = 0, block_start = 0;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,6,0))
		block = bmap(dattobd_get_dentry(dev->sd_lowlevel_io->filp)->d_inode, index);
#else
		block = index;
		ret = bmap(dattobd_get_dentry(dev->sd_lowlevel_io->filp)->d_inode, &block);

		if (ret < 0) {
			LOG_ERROR(ret, "failed to get block number");
			goto error;
		}
#endif
		if (!block) break;
		
		// we need to consider that if physical block number is not continuous for 4K page
		if (index > 0 && block_cnt > 0 && pre_block + 1 != block){
			LOG_DEBUG("not continuous blocks, index : %d, block : %llu, pre_block = %llu", index, block, pre_block);
			block_cnt = 0;
			pre_block = block;
			continue;
		}
		...
		if (++block_cnt != dev->sd_lowlevel_io->blocks_per_page) continue;
		block_cnt = 0;
		block_start = block - dev->sd_lowlevel_io->blocks_per_page + 1;
		
		if (meta_idx < LOWLEVEL_IO_META_COUNT && meta_tail) {
			meta_tail->blocknr = block_start;
			meta_tail = meta_tail->next;
			meta_idx++;
			continue;
		}
		...
		if (range_tail && range_tail->blocknr + (range_tail->total * dev->sd_lowlevel_io->blocks_per_page) == block_start) {
			range_tail->total++;
		} else {
			struct lowlevel_io_range *range = kzalloc(sizeof(struct lowlevel_io_range), GFP_KERNEL);
			
			range->blocknr = block_start;
			range->total = 1;
			range->used = 0;
			...
			if (range_tail) {
				...
				dev->sd_lowlevel_io->range = range;
			}

			range_tail = range;
		}
	}
	dev->sd_lowlevel_io->max_pair_in_meta = data_cnt / LOWLEVEL_IO_META_COUNT;
...
error:
	__tracer_destroy_lowlevel_io(dev);
	if(page_buf) free_page((unsigned long)page_buf);
...
#define __tracer_create_lowlevel_io(dev, cow_path) tracer_setup_lowlevel_io(dev, cow_path, 1)
#define __tracer_setup_lowlevel_io(dev, cow_path) tracer_setup_lowlevel_io(dev, cow_path, 0)

static int __reload_meta(struct lowlevel_io_meta *meta, struct snap_device *dev)
{
	struct page *pg;
	struct bio *bio;
	void *data;
	...
	LOG_DEBUG("reload meta");

#ifdef HAVE_BIO_ALLOC_2
	bio = bio_alloc(GFP_NOIO, 1);
#else
        bio = bio_alloc(dev->sd_base_dev, 1, 0, GFP_KERNEL);
#endif
	if(!bio){
		ret = -ENOMEM;
		LOG_ERROR(ret, "failed to allocate bio");
		goto out;
	}

	bio_sector(bio) = BLOCK_TO_SECTOR_BY_BLOCK_SIZE(meta->blocknr, dev->sd_lowlevel_io->block_size);
	bio_idx(bio) = 0;
	...
	if(!pg){
		ret = -ENOMEM;
		LOG_ERROR(ret, "failed to allocate page");
		goto out;
	}

	len = bio_add_page(bio, pg, PAGE_SIZE, 0);
        if (len != PAGE_SIZE) {
                ret = -EIO;
                LOG_ERROR(ret, "error adding page to bio");
                goto out;
        }
	...
	memcpy(&meta->used, data, sizeof(struct block_pair_count));
	memcpy(meta->addr, data, PAGE_SIZE);
...
static int __read_data_lowlevel(struct snap_device *dev, uint64_t block, struct page *pg)
{
	struct bio *bio;
	int ret, len;
	...
	bio_sector(bio) = BLOCK_TO_SECTOR_BY_BLOCK_SIZE(block, dev->sd_lowlevel_io->block_size);
	bio_idx(bio) = 0;
	dattobd_bio_set_dev(bio, dev->sd_base_dev);
	dattobd_set_bio_ops(bio, REQ_OP_READ, 0);
...
static int __reload_lowlevel_io(struct snap_device *dev)
{
	struct lowlevel_io_meta *meta_tail = dev->sd_lowlevel_io->meta;
	int ret = 0;

	while (meta_tail) {
		ret = __reload_meta(meta_tail, dev);
		if (ret) {
			LOG_ERROR(ret, "failed to reload lowlevel meta");
			break;
		}
...
static int merge_snapshot_lowlevel(struct snap_device *dev)
{
	struct lowlevel_io_meta *meta_tail = dev->sd_lowlevel_io->meta;
	struct page *pg;
	int ret = 0;

	pg = alloc_page(GFP_NOIO | __GFP_ZERO);
	if (!pg) return -ENOMEM;
        LOG_DEBUG("ENTER %s", __func__);

	for (; meta_tail; meta_tail = meta_tail->next) {
		uint64_t size = 0, index = 0;
		void *data = NULL;

		size = meta_tail->used.size;
		if (size == 0 || size > dev->sd_lowlevel_io->max_pair_in_meta) break;

		for (; index < size; index++) {
			struct block_pair *pair = (struct block_pair*)(meta_tail->addr +
				sizeof(struct block_pair_count) + sizeof(struct block_pair) * index);

			ret = __read_data_lowlevel(dev, pair->blocknr, pg);
			if (ret) goto out;

			data = page_address(pg);

			mutex_lock(&dev->sd_cow_mutex);
			ret = cow_write_current(dev->sd_cow, pair->start, data);
			mutex_unlock(&dev->sd_cow_mutex);
			if (ret) goto out;
		}

		if (size != dev->sd_lowlevel_io->max_pair_in_meta) break;
	}
        
        LOG_DEBUG("EXIT  %s", __func__);
out:
	if (pg) __free_page(pg);
	if (ret) LOG_ERROR(ret, "failed to merge snapshot lowlevel");

	return ret;
}

static int merge_inc_lowlevel(struct snap_device *dev)
{
	int ret = 0;
	struct lowlevel_io_meta *meta_tail = dev->sd_lowlevel_io->meta;
	...
	for (; meta_tail; meta_tail = meta_tail->next) {
		...
		for (; index < size; index++) {
			struct block_pair *pair = (struct block_pair*)(meta_tail->addr +
				sizeof(struct block_pair_count) + sizeof(struct block_pair) * index);
			uint64_t start = pair->start;

			for (;start < pair->end; start++) {
				mutex_lock(&dev->sd_cow_mutex);
				ret = cow_write_filler_mapping(dev->sd_cow, start);
				mutex_unlock(&dev->sd_cow_mutex);
				if(ret) return ret;
			}
		}
...
static int __merge_lowlevel(struct snap_device *dev, int reload){
	int ret = 0;

	if (reload) {
		ret = __reload_lowlevel_io(dev);
		if (ret) return ret;
	}

	if (test_bit(SNAPSHOT, &dev->sd_state)) ret = merge_snapshot_lowlevel(dev);
	else ret = merge_inc_lowlevel(dev);

	if(ret) return ret;

	return __clean_lowlevel_io(dev);
}

#define reload_merge_lowlevel(dev) __merge_lowlevel(dev, 1)
#define merge_lowlevel(dev) __merge_lowlevel(dev, 0)
...
static bool bio_is_first_write(struct snap_device *dev, struct bio *bio)
{
        ...
        // LOG_DEBUG("bio sector : %llu, bio size : %u", bio_sector(bio), bio_size(bio));
        // LOG_DEBUG("start sector : %llu, end sector : %llu", sector_start, sector_end);
        // LOG_DEBUG("start block : %llu, end block : %llu", block_start, block_end);
...
static int snap_trace_bio(struct snap_device *dev, struct bio *bio)
{
        ...
        // LOG_DEBUG("start_sect = %llu, end_sect = %llu, pages = %u", start_sect, end_sect, pages);
...
static int __tracer_destroy_cow(struct snap_device *dev, int close_method)
{
        ...
        if (dev->sd_cow) {
                ...
                if (close_method == 0) {
                        ...
                        // need lock to prevent dormant mode lowlevel io write
                        mutex_lock(&dev->sd_cow_mutex);
...
static void __tracer_destroy_base_dev(struct snap_device *dev)
{
        ...
        if (dev->sd_base_file){
                LOG_DEBUG("freeing base block device file");
                fput(dev->sd_base_file);
                dev->sd_base_file = NULL;
                dev->sd_base_dev = NULL;
        }
#elif defined(HAVE_BDEV_OPEN_BY_PATH)
        if (dev->sd_base_handle){
                LOG_DEBUG("freeing base block device & handle");
                bdev_release(dev->sd_base_handle);
                dev->sd_base_handle = NULL;
                dev->sd_base_dev = NULL;
        }
#else
        if (dev->sd_base_dev) {
                                   ...
                                   const char *bdev_path)
{
...
#ifdef HAVE_BDEV_FILE_OPEN_BY_PATH
        dev->sd_base_file = bdev_file_open_by_path(bdev_path, BLK_OPEN_READ, NULL, NULL);
        if (IS_ERR(dev->sd_base_file)) {
                ret = PTR_ERR(dev->sd_base_file);
                dev->sd_base_file = NULL;
                LOG_ERROR(ret, "error opening block device '%s'", bdev_path);
                goto error;
        }
        dev->sd_base_dev = file_bdev(dev->sd_base_file);

#elif defined(HAVE_BDEV_OPEN_BY_PATH)
        dev->sd_base_handle = bdev_open_by_path(bdev_path, FMODE_READ, NULL, NULL);
        if (IS_ERR(dev->sd_base_handle)) {
                ret = PTR_ERR(dev->sd_base_handle);
                dev->sd_base_handle = NULL;
                LOG_ERROR(ret, "error opening block device '%s'", bdev_path);
                goto error;
        }
        dev->sd_base_dev = dev->sd_base_handle->bdev;
        ...
        if (!test_bit(TRACED, &dev->sd_state) && bdev_is_already_traced(dev->sd_base_dev)) {
                ret = -EINVAL;
...
#endif       
                if (dev->sd_queue){
                        LOG_DEBUG("freeing request queue in dev->sd_gd");
...
struct request_queue* (*dattobd_blk_alloc_queue)(int node_id) = (BLK_ALLOC_QUEUE_ADDR != 0) ?
...
// alloc_disk() function has disappeared starting from the kernel 5.15
#ifdef HAVE_ALLOC_DISK
...
#else
	dev->sd_queue = dattobd_blk_alloc_queue(GFP_KERNEL);
#endif
	if(!dev->sd_queue){
		ret = -ENOMEM;
...
#ifdef HAVE_BLK_ALLOC_DISK_2
	// kernel >= 6.9: blk_alloc_disk(lim, node_id) returns ERR_PTR on failure
	dev->sd_gd = blk_alloc_disk(NULL, NUMA_NO_NODE);
	if(IS_ERR(dev->sd_gd)){
		ret = PTR_ERR(dev->sd_gd);
		dev->sd_gd = NULL;
		LOG_ERROR(ret, "error allocating gendisk");
		goto error;
	}
...
static void __tracer_destroy_cow_thread(struct snap_device *dev)
{
        if (dev->sd_cow_thread) {
                LOG_DEBUG("stopping cow thread but first wait for bio complete");
                                     ...
                                     unsigned int minor, int is_snap)
{
        ...
        // cow thread was still running, no need to recreate one
        // (__tracer_dormant_to_active & __tracer_unverified_snap_to_active)
        if (dev->sd_cow_thread) return 0;
...
#endif
{
...
#ifdef HAVE_BDEV_TEST_FLAG
		bdev_set_flag(bdev, BD_HAS_SUBMIT_BIO); // kernel version >= 6.10
#else
		bdev->bd_has_submit_bio = true; // kernel version >= 6.4
                ...
                if (new_bio_tracking_ptr) bdev->bd_disk->queue->make_request_fn = 
                new_bio_tracking_ptr == dattobd_null_mrf ? NULL : new_bio_tracking_ptr;
#else
                if (new_bio_tracking_ptr) bdev->bd_disk->queue->make_request_fn = new_bio_tracking_ptr;
#endif //HAVE_BLK_MQ_MAKE_REQUEST
#else
                if(bd_ops){
                        bdev->bd_disk->fops= bd_ops;
                }
...
#endif
{
        //entry point when read/write for /dev/sdaX, will redirect rw bios to snap_cow_thread and snap_mrf_thread"
        int i, ret = 0;
        ...
        {
                if(!dev || (!test_bit(TRACED, &dev->sd_state) && test_bit(UNVERIFIED, &dev->sd_state)) || !tracer_queue_matches_bio(dev, bio)) continue;
        ...
        // do fops match to find original mrf for the whole gen disk, we need original mrf for this bio to work
	tracer_for_each(dev, i){
		// traced but unverfied, still do bio
		if(!dev || (!test_bit(TRACED, &dev->sd_state) && test_bit(UNVERIFIED, &dev->sd_state)) || !tracer_fops_matches_bio(dev, bio)) continue;
...
#else
        if(orig_fn) {
                ret = dattobd_call_mrf(orig_fn, q, bio);
        }else {
                // find all dev again based on same queue only
                tracer_for_each(dev, i){
                        if(!tracer_is_bio_for_dev_only_queue(dev, bio)) continue;
                ...
                ret = -EFAULT;
                LOG_ERROR(ret, "error finding original_mrf for bio");
...
static int dattobd_find_orig_mrf(struct block_device *bdev,
        ...
        tracer_for_each(dev, i){
                if(!dev || (!test_bit(TRACED, &dev->sd_state) && test_bit(UNVERIFIED, &dev->sd_state))) continue;
		if(tracer_fops_matches_bdev(dev, bdev)){
			if (!dev->sd_orig_request_fn) {
				// match fops and find non null orig mrf bacuase the snap device of current parititon may contain null mrf
				continue;
			}
	...
	tracer_for_each(dev, i){
		...
		if(q == bdev_get_queue(dev->sd_base_dev)){
			if (!dev->sd_orig_request_fn) {
				// match queue and find non null orig mrf bacuase the snap device of current parititon may contain null mrf
				continue;
			}
			*mrf = dev->sd_orig_request_fn;
			return 0;
...
#else
static int find_orig_bdops(struct block_device *bdev, struct block_device_operations **ops, make_request_fn **mrf, struct tracing_ops** trops){
        int i;
	struct snap_device *dev;
...
void wait_for_bio_complete(struct snap_device *dev)
{
                             ...
                             unsigned long cache_size)
{
        ...
        //setup lowlevel io
        ret = __tracer_create_lowlevel_io(dev, cow_path);
        if(ret) goto error;
...
int tracer_active_snap_to_inc(struct snap_device *old_dev)
{
        ...
        __tracer_copy_lowlevel_io(old_dev, dev);
...
{
        info->is_cancelled = dev->sd_cow ? snap_is_cancelled(dev->sd_cow) : false;
        LOG_DEBUG("info->is_cancelled : %d", info->is_cancelled);
...
/**
 * tracer_dattobd_lowlevel_info() - Copies relevant, current information abound lowlevel in @dev to
 *                         @lowlevel_info.
 *
 * @dev: The source &struct snap_device tracking block device state.
 * @lowlevel_info: A destination &struct dattobd_lowlevel_info object pointer.
 */
void tracer_dattobd_lowlevel_info(const struct snap_device *dev,
                         struct dattobd_lowlevel_info *lowlevel_info)
{
        struct lowlevel_io_meta *meta_tail = dev->sd_lowlevel_io->meta;
        unsigned int nr_changed_blocks = 0;

        lowlevel_info->minor = dev->sd_minor;
		...
		for (; index < size; index++) {
			struct block_pair *pair = (struct block_pair*)(meta_tail->addr +
				sizeof(struct block_pair_count) + sizeof(struct block_pair) * index);
                        LOG_DEBUG("nr_changed_blocks : %d, pair->start : %llu", nr_changed_blocks, pair->start);
                        if (nr_changed_blocks < 2040) {
                                lowlevel_info->changed_blocks[nr_changed_blocks] = pair->start;
                                nr_changed_blocks++;
                        }
        ...
        LOG_DEBUG("nr_changed_blocks : %d", nr_changed_blocks);
        lowlevel_info->nr_changed_blocks = nr_changed_blocks;
}

int tracer_sync_cow(struct snap_device *dev)
{
        int ret;
        mutex_lock(&dev->sd_cow_mutex);
        ret = __cow_sync_sections(dev->sd_cow, 0);
        mutex_unlock(&dev->sd_cow_mutex);
        if (ret) {
                LOG_ERROR(ret, "error syncing cow from ioctl");
        }
        return ret;
}
...
void __tracer_active_to_dormant(struct snap_device *dev)
{
        ...
        //close lowlevel io filp
        file_close(dev->sd_lowlevel_io->filp);
        dev->sd_lowlevel_io->filp = NULL;
 ...
 *
 * @dev: The &struct snap_device object pointer.
 * @user_mount_path: A path used to build the COW file path.
 * @is_user_space: A flag indicating if the path is from userspace.
 */
void __tracer_unverified_snap_to_active(struct snap_device *dev,
                                        const char *user_mount_path,
                                        int is_user_space)
{
        int ret;
        ...
        if (!test_bit(TRACED, &dev->sd_state)) __tracer_destroy_tracing(dev);

        // mark as active
        set_bit(ACTIVE, &dev->sd_state);
        clear_bit(UNVERIFIED, &dev->sd_state);

        if (!test_bit(TRACED, &dev->sd_state)) {
            bdev_path = dev->sd_bdev_path;
            rel_path = dev->sd_cow_path;
            dev->sd_bdev_path = NULL;
            dev->sd_cow_path = NULL;
            ...
            bdev_path = kstrdup(dev->sd_bdev_path, GFP_KERNEL);
            if(!bdev_path){
                ret = -ENOMEM;
                goto error;
            }

            rel_path = kstrdup(dev->sd_cow_path, GFP_KERNEL);
            if(!rel_path){
                ret = -ENOMEM;
                goto error;
            }
        ...
        ret = user_mount_pathname_concat(user_mount_path, rel_path, &cow_path, is_user_space);
        if (ret)
                goto error;
        ...
        // setup lowlevel io if not exist
        ret = __tracer_setup_lowlevel_io(dev, cow_path);
        if(ret) goto error;

        // reload and merge lowlevel traced block
        ret = reload_merge_lowlevel(dev);
        if(ret) goto error;
                                       ...
                                       int is_user_space)
{
        ...
        // setup lowlevel if not exist
        LOG_DEBUG("call __tracer_setup_lowlevel_io in __tracer_unverified_inc_to_active");
        ret = __tracer_setup_lowlevel_io(dev, cow_path);
        if(ret) goto error;
                                ...
                                int is_user_space)
{
        ...
        // generate the full pathname
        ret = user_mount_pathname_concat(user_mount_path, dev->sd_cow_path,  &cow_path, is_user_space);
        if (ret)
                goto error;
        mutex_lock(&dev->sd_cow_mutex);
        // setup the cow manager
        ...
        //reopen lowlevel io filp
        ret = __open_lowlevel_io_file(cow_path, &dev->sd_lowlevel_io->filp);
        if(ret) goto error;

        // merge lowlevel traced block
        // if already in fail state, we should not merge lowlevel
	if (tracer_read_fail_state(dev)) ret = __clean_lowlevel_io(dev);
	else ret = merge_lowlevel(dev);
        if(ret) goto error;
...
int setup_traced(const char *dev_name)
{
	int ret, i, len;
        char *upath = NULL;
	struct snap_device *dev = NULL;
        LOG_DEBUG("enter setup_traced");

        ret = pathname_to_absolute(dev_name, &upath, &len);
	if(!upath || ret) goto out;
        
	LOG_DEBUG("absolute path: %s(%s), len: %d", upath, dev_name, len);

	tracer_for_each(dev, i) {
		if(!dev || test_bit(SNAPSHOT, &dev->sd_state) || !test_bit(UNVERIFIED, &dev->sd_state) || upath[0] != '/') continue;
		LOG_DEBUG("dev->sd_bdev_path : %s", dev->sd_bdev_path);
                if (memcmp(dev->sd_bdev_path, upath, min((int)strlen(dev->sd_bdev_path), len)) != 0) {
			continue;
		}

                LOG_DEBUG("found block device in setup_traced");
		ret = __tracer_setup_base_dev(dev, dev_name);
		if(ret) goto error;

		set_bit(TRACED, &dev->sd_state);

#ifndef USE_BDOPS_SUBMIT_BIO
                LOG_DEBUG("setting dattobd_find_orig_mrf in setup_traced");
                ret = dattobd_find_orig_mrf(dev->sd_base_dev, &dev->sd_orig_request_fn);
                if (ret) goto error;
                // inject traing mrf
                dev->sd_base_dev->bd_disk->queue->make_request_fn = tracing_fn;
#else
                LOG_DEBUG("find and set bdops in setup_traced");
                if(!dev->sd_tracing_ops){
                        ret = find_orig_bdops(dev->sd_base_dev, &dev->bd_ops,&dev->sd_orig_request_fn, &dev->sd_tracing_ops);
                        if(ret) goto error;
                        ...
                        dev->sd_base_dev->bd_disk->fops = dev->sd_tracing_ops->bd_ops;
#ifdef HAVE_BD_HAS_SUBMIT_BIO
#ifdef HAVE_BDEV_TEST_FLAG
                        bdev_set_flag(dev->sd_base_dev, BD_HAS_SUBMIT_BIO);
#else
                        dev->sd_base_dev->bd_has_submit_bio = true; // kernel version >= 6.4
#endif
#endif
                }
                else {
                        LOG_DEBUG("device with minor %i already has sd_tracing_ops", i);
                }
...
out:
        LOG_DEBUG("exit setup_traced");
	if(upath) kfree(upath);
	return 0;

error:
	if(upath) kfree(upath);
	if(dev && dev->sd_base_dev) __tracer_destroy_base_dev(dev);
	if(dev) dev->sd_orig_request_fn = NULL;
	if(dev) clear_bit(TRACED, &dev->sd_state);
