...
#define LOG_DEBUG(fmt, args...) \
	do{ \
		if(elastio_snap_debug) printk(KERN_DEBUG "synosnap: " fmt "\n", ## args); \
	}while(0)

#define LOG_WARN(fmt, args...) printk(KERN_WARNING "synosnap: " fmt "\n", ## args)
#define LOG_ERROR(error, fmt, args...) printk(KERN_ERR "synosnap: " fmt ": %d\n", ## args, error)
...
#define INFO_PROC_FILE "synosnap-info"
#define DRIVER_NAME "synosnap"
#define CONTROL_DEVICE_NAME "synosnap-ctl"
#define SNAP_DEVICE_NAME "synosnap%u"
#define SNAP_COW_THREAD_NAME_FMT "syno_snap_cow%d"
#define SNAP_MRF_THREAD_NAME_FMT "syno_snap_mrf%d"
#define INC_THREAD_NAME_FMT "syno_inc%d"
...
#define LOW_MEMORY_FAIL_PERCENT 5
...
#define COW_MAX_SIZE ULONG_MAX
#define snap_is_cancelled(cm) (cm->curr_pos == COW_MAX_SIZE)
#define snap_cancel(cm) (cm->curr_pos = COW_MAX_SIZE)
...
static int user_mount_pathname_concat(const char *user_mount_path, const char *rel_path, char **path_out, int is_user_space){
	int ret;
	char *mount_path;
	...
	if (is_user_space){
		...
		mount_path = kstrdup(user_mount_path, GFP_KERNEL);
		if (!mount_path) {
			ret = -ENOMEM;
			goto error;
		}
...
static int __cow_file_extents_zero_fill_ahead(struct cow_manager *cm)
{
	...
	if (!buf) {
		remaining = write_ahead;
		/* falls back when error allocating a 4MB contiguous buffer with kmalloc.
		* Instead allocate a single zeroed page and write in PAGE_SIZE chunks.
		* This avoids high-order allocation failures under memory fragmentation.
		*/
		page = (char *)get_zeroed_page(GFP_KERNEL);
		if (!page) {
			ret = -ENOMEM;
			LOG_ERROR(ret, "error allocating zeroed page for write-ahead");
			return ret;
		}

		while (remaining != 0) {
			uint64_t chunk = min((uint64_t)PAGE_SIZE, remaining);

			ret = file_write(cm, page, curr_size, chunk);
			if (ret < 0) {
				free_page((unsigned long)page);
				LOG_ERROR(ret, "error writing zero page during write-ahead");
				return ret;
			}
...
static int __cow_write_data(struct cow_manager *cm, void *buf){
	...
	if (snap_is_cancelled(cm)) return 0;
	...
	if(curr_size >= cm->file_max) {
		...
		if(!abs_path){
			LOG_ERROR(ret, "cow file max size exceeded (%llu/%llu), data will be cancelled.", curr_size, cm->file_max);
		}else{
			LOG_ERROR(ret, "cow file '%s' max size exceeded (%llu/%llu), data will be cancelled.", abs_path, curr_size, cm->file_max);
			kfree(abs_path);
		}
	...
	while(!kthread_should_stop() || !bio_queue_empty(bq) || atomic64_read(&dev->sd_submitted_cnt) != atomic64_read(&dev->sd_received_cnt)) {
		...
		*/
		...
		if(!bio_data_dir(bio)){
			...
			if (dev->sd_cow && snap_is_cancelled(dev->sd_cow)) {
				LOG_DEBUG("cancelling read bio: sect = %llu size = %u", (unsigned long long)bio_sector(bio), bio_size(bio) / 512);
				elastio_snap_bio_endio(bio, -EFBIG);
				continue;
			}
...
static MRF_RETURN_TYPE snap_mrf(struct bio *bio){
	...
	if (bio_data_dir(bio)) {
		...
		elastio_snap_bio_endio(bio, -EBUSY);
		MRF_RETURN(0);
	} else if(dev->sd_cow && snap_is_cancelled(dev->sd_cow)){
		elastio_snap_bio_endio(bio, -EFBIG);
		MRF_RETURN(0);
	}
...
static int find_orig_mrf(struct block_device *bdev, make_request_fn **mrf){
	...
	*mrf = NULL;
	LOG_ERROR(-EFAULT, "error finding original mrf for bdev");
	return -EFAULT;
}
#endif
...
static int find_orig_fops(struct block_device *bdev, struct block_device_operations **ops, make_request_fn **mrf, struct tracing_ops **tracing_ops){
	...
	*ops = NULL;
	*mrf = NULL;
	LOG_ERROR(-EFAULT, "error finding original mrf and bd_ops for bdev");
	return -EFAULT;
}
#endif
...
#ifdef HAVE_FREEZE_SUPER_3
		ret = freeze_super(origsb, FREEZE_HOLDER_KERNEL, NULL);
#elif defined HAVE_FREEZE_SUPER_2
...
static void __tracer_init(struct snap_device *dev){
	...
	/* Ensure request queue isn't ignored for freshly initialized device */
...
static int __tracer_setup_cow(struct snap_device *dev, struct block_device *bdev, const char *cow_path,
...
static int __tracer_setup_cow_thread(struct snap_device *dev, unsigned int minor, int is_snap){
	...
	/* Ensure request queue is enabled after (re)setup in case it was stopped earlier */
	atomic_set(&dev->sd_ignore_requests, 0);
...
static void __tracer_unverified_snap_to_active(struct snap_device *dev, const char *user_mount_path, int is_user_space){
	int ret;
...
static void __tracer_unverified_inc_to_active(struct snap_device *dev, const char *user_mount_path, int is_user_space){
	int ret;
	unsigned int minor = dev->sd_minor;
	char *cow_path;
	char *bdev_path = dev->sd_bdev_path, *rel_path = dev->sd_cow_path;
...
static void __tracer_dormant_to_active(struct snap_device *dev, const char *user_mount_path, int is_user_space){
	int ret;
	char *cow_path;
...
static void auto_transition_active(unsigned int i, const char *dir_name, int is_user_space){
	...
	if(test_bit(UNVERIFIED, &dev->sd_state)){
		if(test_bit(SNAPSHOT, &dev->sd_state)) __tracer_unverified_snap_to_active(dev, dir_name, is_user_space);
		else __tracer_unverified_inc_to_active(dev, dir_name, is_user_space);
	}else __tracer_dormant_to_active(dev, dir_name, is_user_space);
...
static int __handle_bdev_mount_nowrite(const struct vfsmount *mnt, unsigned int *idx_out){
	...
	tracer_for_each(dev, i){
		...
		if (dev && dev->sd_cow && mnt == elastio_snap_get_mnt(dev->sd_cow->filp)) {
...
static int __handle_bdev_mount_writable(const char *dir_name, const struct block_device *bdev, unsigned int *idx_out, int is_user_space){
	int ret;
	unsigned int i;
	struct snap_device *dev;
	struct bdev_container bd_c;
...
static int handle_bdev_mount_event(const char *dir_name, int follow_flags, unsigned int *idx_out, int mount_writable, int is_user_space){
	...
	else ret = __handle_bdev_mount_writable(dir_name, bdev, idx_out, is_user_space);
	if(ret){
...
static int __maybe_unused setup_traced(const char *dev_name)
{
	int ret, i, len;
	char *upath = NULL;
	struct snap_device *dev = NULL;
	...
	tracer_for_each(dev, i) {
		if(!dev || test_bit(SNAPSHOT, &dev->sd_state) || !test_bit(UNVERIFIED, &dev->sd_state) || tracer_read_fail_state(dev) || upath[0] != '/') continue;
		...
		minor = dev->sd_minor;
		cache_size = dev->sd_cache_size;

		bdev_path = dev->sd_bdev_path; // moved
		// dev->sd_bdev_path = NULL; // concurrent mount may read this value
...
#ifndef USE_BDOPS_SUBMIT_BIO
		LOG_DEBUG("setting find_orig_mrf in setup_traced");
		ret = find_orig_mrf(dev->sd_base_dev, &dev->sd_orig_mrf);
		if (ret) goto error;
		// inject traing mrf
		dev->sd_base_dev->bd_disk->queue->make_request_fn = tracing_mrf;
#else
...
error:
	LOG_ERROR(ret, "error in setup_traced");
	if(upath) kfree(upath);
	if(dev && dev->sd_base_dev) __tracer_destroy_base_dev(dev);
	if(dev) dev->sd_orig_mrf = NULL;
	if(dev) clear_bit(TRACED, &dev->sd_state);
	if(dev) {
		char *cow_path = dev->sd_cow_path;
		tracer_setup_unverified_inc(dev, minor, bdev_path, cow_path, cache_size, dev->sd_ignore_snap_errors);
		tracer_set_fail_state(dev, ret);
		kfree(cow_path);
	}
	if(bdev_path) kfree(bdev_path);
...
#define handle_bdev_mount_nowrite_kernel(dir_name, follow_flags, idx_out) handle_bdev_mount_event(dir_name, follow_flags, idx_out, 0, 0)
#define handle_bdev_mount_nowrite_user(dir_name, follow_flags, idx_out) handle_bdev_mount_event(dir_name, follow_flags, idx_out, 0, 1)

#define handle_bdev_mounted_writable_kernel(dir_name, idx_out) handle_bdev_mount_event(dir_name, 0, idx_out, 1, 0)
#define handle_bdev_mounted_writable_user(dir_name, idx_out) handle_bdev_mount_event(dir_name, 0, idx_out, 1, 1)

static void post_umount_check(int dormant_ret, long umount_ret, unsigned int idx, const char *dir_name, bool is_user_space){
	struct bdev_container bd_c;
	struct block_device *bdev;
	struct snap_device *dev;
	struct super_block *sb;
...
struct mount {
	...
	struct vfsmount mnt;
	union {
#if defined(HAVE_BDEV_FREEZE) && !defined(MNT_ONRB)
...
static asmlinkage long mount_hook(char __user *dev_name, char __user *dir_name, char __user *type, unsigned long flags, void __user *data){
	...
	if(ret_dev || ret_dir)
		LOG_DEBUG("detected block device Get mount params error!, %d, %d, 0x%lx", ret_dev, ret_dir, flags);
	else
	...
	if(real_flags & (MS_BIND | MS_SHARED | MS_PRIVATE | MS_SLAVE | MS_UNBINDABLE | MS_MOVE) || ((real_flags & MS_RDONLY) && !(real_flags & MS_REMOUNT))){
		...
		LOG_DEBUG("detected remount to read-only mount");
...
static asmlinkage long fsconfig_hook(struct pt_regs *regs){
	...
	LOG_DEBUG("start to find fs's mount point in mnt_namespace list");
...
#ifdef USE_NEW_MOUNT_API
	HOOK("__x64_sys_mount_setattr", mount_setattr_hook, &orig_mount_setattr),
	HOOK("__x64_sys_move_mount", move_mount_hook, &orig_move_mount),
	HOOK("__x64_sys_fsconfig", fsconfig_hook, &orig_fsconfig),
#else
	HOOK("__x64_sys_mount", mount_hook, &orig_mount),
#endif
	HOOK("__x64_sys_umount", umount_hook, &orig_umount),
};
...
static unsigned long lookup_name(const char *name)
{
	...
	LOG_DEBUG("using kprobe to find address of name : %s: %lx", name, address);
...
static unsigned long lookup_name(const char *name)
{
	...
	LOG_DEBUG("kallsyms_lookup_name address of name : %s: %lx", name, address);
	return address;
}
...
static int register_ftrace_hooks(void)
{
	int ret = 0;
	int i;
...
static int unregister_ftrace_hooks(void)
{
	int ret = 0;
	int i;
	int count = ARRAY_SIZE(ftrace_hooks);
		...
		if(!test_bit(UNVERIFIED, &dev->sd_state)){
			...
			if(dev->sd_cow){
				...
				if(dev->sd_cow->version > COW_VERSION_0){
					...
					seq_printf(m, "\t\t\t\"cancellation mode\": %d,\n", snap_is_cancelled(dev->sd_cow));
				}
			}
		}
...
#if X64_SYS_CALL_ADDR
	LOG_DEBUG("unregistering ftrace hooks");
	unregister_ftrace_hooks();
#else
	LOG_DEBUG("restoring system call tables");
...
#if X64_SYS_CALL_ADDR
		LOG_DEBUG("using ftrace hooks insteam of system call table");
		ret = register_ftrace_hooks();
		if (ret) {
			LOG_ERROR(ret, "ftrace hooks failed");
			goto error;
		}
#else
