...
dpkg-deb -I "$TMPDIR/Synology Active Backup for Business Agent-3.1.0-4968.deb" control 2>/dev/null | grep Version || echo "Agent DEB not found"

echo ""
echo "=== Verifying binary patch ==="
AGENTDIR=$(mktemp -d)
dpkg-deb -R "$TMPDIR/Synology Active Backup for Business Agent-3.1.0-4968.deb" "$AGENTDIR" 2>/dev/null
grep -ao "build.4968" "$AGENTDIR/bin/abb-cli" 2>/dev/null && echo "abb-cli: PATCHED" || echo "abb-cli: NOT patched"
grep -ao "build.4968" "$AGENTDIR/opt/Synology/ActiveBackupforBusiness/bin/service-ctrl" 2>/dev/null && echo "service-ctrl: PATCHED" || echo "service-ctrl: NOT patched"
grep -ao "build.4968" "$AGENTDIR/opt/Synology/ActiveBackupforBusiness/bin/synology-backupd" 2>/dev/null && echo "synology-backupd: PATCHED" || echo "synology-backupd: NOT patched"
